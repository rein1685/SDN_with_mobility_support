...
static int count_for_insert_flowtable = 0;
...
void SwPort::packet_handler(u_char *user, const struct pcap_pkthdr* pkthdr,const u_char*
		...
		if(count_for_insert_flowtable == 0)
		{
			Flow flow;		//if dst port = 6666 , OFPP_UDP_VENDOR action call
			of10::Match m;
			ActionList act_list;
			of10::OutputAction act(of10::OFPP_UDP_VENDOR , 1024);

			m.tp_dst(6666);

			act_list.add_action(act);

			flow.priority_ = 100;
			flow.match = m;
			flow.actions = act_list;							
		
               		for(std::set<Flow>::iterator it = (*(pcap_dp->dp))->flow_table.begin();
               	        it != (*(pcap_dp->dp))->flow_table.end(); ++it)
			{
				if(Flow::strict_match(*it , flow)){
					(*(pcap_dp->dp))->flow_table.erase(it);
					break;
				}
			}

			(*(pcap_dp->dp))->flow_table.insert(flow);
		}
